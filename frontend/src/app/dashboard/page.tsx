      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <Link href="/" className="p-2 glass rounded-full hover:bg-amber-500/10 transition-colors border border-amber-500/20 text-amber-400">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            <h1 className="text-3xl font-bold flex items-center gap-3 text-zinc-100">
              <LayoutDashboard className="text-amber-400" /> AI Dashboard
            </h1>
          </div>
          
          {/* User Profile & Logout */}
          <div className="flex items-center gap-4 glass px-4 py-2 rounded-full border-amber-500/20 shadow-[0_0_15px_rgba(218,165,32,0.05)]">
            <div className="flex items-center gap-2 pr-4 border-r border-white/10">
              <div className="w-8 h-8 rounded-full bg-gradient-to-r from-amber-400 to-yellow-600 flex items-center justify-center text-black font-bold">
            ...
            <button 
              ...
              className="text-zinc-400 hover:text-amber-400 transition-colors flex items-center gap-2 text-sm font-medium"
            >
        ...
        <div className="grid lg:grid-cols-3 gap-8">
              ...
              <>
                ...
                <div className="glass p-8 rounded-2xl">
                  <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
                    <Target className="text-amber-500" /> Executive Summary
                  </h2>
                ...
                {data.important_metrics && Object.keys(data.important_metrics).length > 0 && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {Object.entries(data.important_metrics).map(([key, val], idx) => (
                      <div key={idx} className="glass p-6 rounded-xl flex flex-col justify-center border-t-4 border-t-amber-500">
          ...
          <div className="lg:col-span-1 h-[800px]">
            <div className="glass rounded-2xl h-full flex flex-col border border-white/10 sticky top-8">
              <div className="p-4 border-b border-amber-500/20 bg-black/40 rounded-t-2xl flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-amber-400 to-yellow-600 flex items-center justify-center shadow-[0_0_10px_rgba(218,165,32,0.5)]">
                  <MessageSquare className="text-black w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-bold text-zinc-100">AI Analyst Agent</h3>
                  <p className="text-xs text-amber-400 flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" /> Online
                  </p>
                </div>
              </div>
                ...
                {chatHistory.map((msg, i) => (
                  <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`p-3 rounded-2xl text-sm border border-white/10 max-w-[85%] ${
                      msg.role === 'user' 
                        ? 'bg-amber-600/30 rounded-tr-none ml-auto border-amber-500/30' 
              ...
              <div className="p-4 border-t border-amber-500/20 bg-black/20 rounded-b-2xl">
                <div className="relative">
                  <input 
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-full py-3 pl-4 pr-12 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 placeholder:text-muted-foreground"
                  />
                  <button 
                    onClick={handleChat}
                    className="absolute right-2 top-2 p-1.5 rounded-full bg-amber-600 text-black hover:bg-amber-500 transition-colors"
                  >
